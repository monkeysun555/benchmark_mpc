...
if not IF_ALL_TESTING:
	...
	LOG_FILE = LOG_FILE_DIR + '/MPCLAT_' + str(int(SERVER_START_UP_TH/MS_IN_S)) + 's'
else:
	if IF_NEW:
		...
		for i in range(TEST_DURATION):
			...
			while True:  # serve video forever
				...
				real_chunk_size, download_duration, freezing, time_out, player_state = player.fetch(download_chunk_size, 
																		download_seg_idx, download_chunk_idx, take_action, chunk_number, SPEED[speed])
				take_action = 0
				...
				if server.check_chunks_empty():
					...
					player.wait(server_wait_time, SPEED[speed])
