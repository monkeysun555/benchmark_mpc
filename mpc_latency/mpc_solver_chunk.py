...
def mpc_solver_chunk(mpc_input):
	...
	for i in range(len(BITRATE)):
		...
		for j in range(len(SPEED)):
			...
			last_speed = mpc_input[8]
			pre_reward = mpc_input[9]
			seq = mpc_input[10][:]
			speed_seq = mpc_input[11][:]
			buffer_to_accu = 0.0
			if state == 0:
			...
			while True:
				...
				if state == 0:
					...
					assert buffer_to_accu == 0.0
					freezing = np.maximum(0.0, download_time * SPEED[current_speed] - buffer_length)
					playback_time += np.minimum(buffer_length, download_time * SPEED[current_speed])
					buffer_length = np.maximum(buffer_length - download_time * SPEED[current_speed], 0.0)
					buffer_length += CHUNK_DURATION * chunk_num
					if freezing > 0.0:
