...
if not IF_ALL_TESTING:
	...
	if IF_NEW:
		...
		ALL_TESTING_FILE = ALL_TESTING_DIR + 'MPC\'_cp_' + str(int(SERVER_START_UP_TH/MS_IN_S)) + 's.txt'
	else:
...
def t_main():
	...
	for i in range(len(cooked_times)):
		...
		for i in range(TEST_DURATION):
			...
			mpc_tp_pred = [x*KB_IN_MB for x in cooked_bw[i:i+MPC_STEP]]
