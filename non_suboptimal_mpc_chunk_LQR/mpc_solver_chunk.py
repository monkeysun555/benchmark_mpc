...
def predict_mpc_tp(updated_pred_tp, steps = MPC_STEP):
	assert len(updated_pred_tp) == steps
	combined_tp = updated_pred_tp[:]
	combined_tp.extend(np.zeros(steps))
	for i in range(steps):
		combined_tp[steps + i] = harmonic_prediction(combined_tp[i:i+steps])
	return combined_tp[-steps:]

def harmonic_prediction(history):
	return len(history)/(np.sum([1/tp for tp in history]))

def predict_mpc_tp_noisy(updated_pred_tp, mu, sigma):
