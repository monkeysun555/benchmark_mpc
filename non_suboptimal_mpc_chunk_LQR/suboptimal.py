...
SUMMARY_DIR = './subopt_curve/'
TOTAL_PATH = './subopt_curve/plotall_data.txt'
TYPES = [1, 2, 3]
...
def get_bw_trace(cooked_bw, index, lookahead):
	if index + lookahead < len(cooked_bw):
		return [x*KB_IN_MB for x in cooked_bw[index:index+lookahead]]
	else:
		len_left = index + lookahead - len(cooked_bw)
		pre = [x*KB_IN_MB for x in cooked_bw[index:]]
		pre.extend([x*KB_IN_MB for x in cooked_bw[:len_left]])
		return pre

def find_subopt(temp_type, server_start_up, curr_dir):

	if temp_type == 1:			# Sensitive to latency
		ACTION_REWARD = 1.0 * CHUNK_SEG_RATIO	
		REBUF_PENALTY = 6.0		# for second
		SMOOTH_PENALTY = 1.0
	...
	elif temp_type == 2:			# Sensitive to bitrate
	...
	for lh in LH_STEPS:
		...
		for i in range(len(cooked_times)):
			...
			player = live_player.Live_Player(time_trace=cooked_time, throughput_trace=cooked_bw, 
												...
												start_up_th=USER_START_UP_TH, freezing_tol=USER_FREEZING_TOL, latency_tol = server_start_up + USER_FREEZING_TOL,
			...
			for i in range(TEST_DURATION):
				...
				mpc_tp_pred = mpc.predict_mpc_tp(mpc_tp_rec, lh)
...
def main():
	...
	for t in TYPES:
		for buffer_len in BUFFERS:
			curr_dir = SUMMARY_DIR + 'type_' + str(t) + '/buffer_len_' + str(buffer_len/MS_IN_S) + '/' 
			if not os.path.exists(curr_dir):
				os.makedirs(curr_dir)
			processes.append(Process(target=find_subopt, args=(t, buffer_len, curr_dir)))
			processes[-1].start()
